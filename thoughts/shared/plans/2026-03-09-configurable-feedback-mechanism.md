...
{#each reasons as reason}
	<button ...>
		{reasonLabels[reason] ??
			$i18n.t(reason.replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase()))}
	</button>
{/each}
...
{#if layer3Enabled}
	<textarea placeholder={layer3Prompt || $i18n.t('Feel free to add specific details')} ... />
{/if}
```
...
{#if enabled}
	<div class="w-full max-w-4xl mx-auto px-4" transition:fade={{ duration: 150 }}>
		{#if !expanded}
			...
			>
				...
				<span class="whitespace-nowrap"
					>{submitted ? $i18n.t('Feedback submitted') : $i18n.t('How was this conversation?')}</span
				>
				<div
					class="flex-1 h-px bg-gray-200 dark:bg-gray-700 group-hover:bg-gray-300 dark:group-hover:bg-gray-600 transition"
				/>
		...
		{:else}
			...
			<div
				class="border border-gray-100/30 dark:border-gray-850/30 rounded-xl px-4 py-3 mb-1"
		...
		<div class="ml-2 mb-3">
			...
			{#each evaluationConfig.FEEDBACK_LAYER2_TAGS ?? [] as tag, index}
				<div class="flex items-center gap-2 mb-1.5">
					...
					<button
						...
						on:click={() => {
							evaluationConfig.FEEDBACK_LAYER2_TAGS = evaluationConfig.FEEDBACK_LAYER2_TAGS.filter(
								(_, i) => i !== index
							);
						}}
					>
			...
			<button
				...
				on:click={() => {
					...
					evaluationConfig.FEEDBACK_LAYER2_TAGS = [
						...evaluationConfig.FEEDBACK_LAYER2_TAGS,
						{ key, label: '' }
					];
				}}
