...
from sqlalchemy import BigInteger, Boolean, Column, String, Text, ForeignKey, select, delete, func
...
class RecoveryCodesTable:
    async def generate_codes(self, user_id: str, db: Optional[AsyncSession] = None) -> list[str]:
        ...
        async with get_async_db_context(db) as db:
            ...
            await db.execute(delete(RecoveryCode).filter(RecoveryCode.user_id == user_id))
