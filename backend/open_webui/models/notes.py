...
class NoteTable:
    ...
    async def delete_notes_by_user_id(self, user_id: str, db: Optional[AsyncSession] = None) -> bool:
        """Delete all notes for a user. Used by GDPR retention/archival workers.
        Converted to async during the v0.9.5 upstream merge to match the rest
        of NoteTable."""
        try:
            async with get_async_db_context(db) as db:
                await db.execute(delete(PinnedNote).filter(PinnedNote.user_id == user_id))
                await db.execute(delete(Note).filter(Note.user_id == user_id))
                await db.commit()
                return True
        except Exception:
            return False
