...
# ---------------------------------------------------------------------------
# HA loop-safety core (Gradient-DS)
# ---------------------------------------------------------------------------
...
# Backwards-compatible alias for the fork's callers (socket layer, tests).
...
def get_redis_connection(
    ...
    HA loop-safety: an async client requested before the FastAPI lifespan has
    flushed the cache is likely bound to the wrong event loop. We warn rather
    than fail so callers can migrate to lifespan-time construction.
    """
