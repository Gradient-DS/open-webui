	...
	<div slot="content">
		<DropdownMenu className="w-70 max-h-72 overflow-hidden transition">
			{#if tab === ''}
				...
				>
					{#if toolPermissionsEnabled && isFeatureEnabled('tools') && itemAllowed('tools')}
						<button
					...
					<!-- [Gradient] Attachment visibility and data separation. -->
					{#if itemAllowed('upload_files')}
						<Tooltip
							content={internalBlocked
					...
					<!-- [Gradient] Attachment visibility and data separation. -->
					{#if itemAllowed('capture') && isFeatureEnabled('capture')}
						<Tooltip
							content={internalBlocked
								? dataSeparationMessage
								: fileUploadCapableModels.length !== selectedModels.length
					...
					<!-- [Gradient] Attachment visibility and data separation. -->
					{#if itemAllowed('attach_webpage') && isFeatureEnabled('webpage_url')}
						<Tooltip
						...
						>
							...
							>
								<GlobeAlt />
								<div class="line-clamp-1">{$i18n.t('Webpage URL')}</div>
							</button>
						</Tooltip>
					{/if}
					...
					<!-- [Gradient] Attachment visibility and data separation. -->
					{#if itemAllowed('knowledge') && isFeatureEnabled('knowledge')}
						<Tooltip
							content={internalBlocked
								? dataSeparationMessage
								: fileUploadCapableModels.length !== selectedModels.length
					...
					<!-- [Gradient] Attachment visibility and data separation. -->
					{#if itemAllowed('reference_chats') && isFeatureEnabled('reference_chats')}
						<Tooltip
							content={internalBlocked
								? dataSeparationMessage
								: fileUploadCapableModels.length !== selectedModels.length
					...
					{#if fileUploadEnabled}
						...
						{#if itemAllowed('onedrive') && $config?.features?.enable_onedrive_integration && ($config?.features?.enable_onedrive_personal || $config?.features?.enable_onedrive_business)}
							<Tooltip content={internalBlocked ? dataSeparationMessage : ''} className="w-full"
								><button
									class:opacity-50={internalBlocked}
									aria-disabled={internalBlocked}
