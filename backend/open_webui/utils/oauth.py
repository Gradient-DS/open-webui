...
OAUTH_RUNTIME_CONFIG = {
    ...
    'OAUTH_INVITE_REQUIRED': ('auth.invite_required', OAUTH_INVITE_REQUIRED),
    'OAUTH_REFRESH_TOKEN_INCLUDE_SCOPE': (
    ...
    'OAUTH_ALLOWED_TENANTS': ('oauth.allowed_tenants', OAUTH_ALLOWED_TENANTS),
...
class OAuthManager:
    ...
    async def handle_callback(self, request, provider, response, db=None):
        ...
        try:
            ...
            if (
                not pending_invite
                and '*' not in auth_config.OAUTH_ALLOWED_DOMAINS
            ...
            if user:
                ...
                if not consumed_invite:
                    ...
                    if not auth_config.ENABLE_OAUTH_SIGNUP:
                        raise HTTPException(
                            status.HTTP_403_FORBIDDEN,
                            detail=ERROR_MESSAGES.ACCESS_PROHIBITED,
                        )
                ...
                if existing_user:
                    ...
                    # `get_user_role` became async in the v0.9.0 refactor — the
                    # `await` here is required for the conditional below it to
                    # carry a role string instead of a coroutine.
                    role = consumed_invite.role if consumed_invite else await self.get_user_role(None, user_data)
