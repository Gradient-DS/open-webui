...
class UserArchiveTable:
    ...
    async def get_archives(
        ...
        async with get_async_db_context(db) as db:
            ...
            stmt = stmt.order_by(UserArchive.created_at.desc()).offset(skip).limit(limit)
            result = await db.execute(stmt)
            return [UserArchiveSummaryModel.model_validate(a) for a in result.scalars().all()]
    ...
    async def get_expired_archives(self, db: Optional[AsyncSession] = None) -> List[UserArchiveModel]:
        ...
        async with get_async_db_context(db) as db:
            ...
            return [UserArchiveModel.model_validate(a) for a in result.scalars().all()]
