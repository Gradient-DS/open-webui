...
2. **COMPLETED — full-stack local e2e environment (stack-1)**: warren pipeline LIVE as compose project `stack-1-dp` (genai-utils worktree): rabbitmq :18190/:18191, mongodb :18192 (db owui_dp), redis :18193, minio :18195/:18196 (bucket owui), pipeline-api :18194, workers parser/chunker/owui-ingest/publication/job-status/retry. tmux windows `dp-logs` + `rmq` added to session `stack-1`; owui-be restarted with pipeline env. E2E PROVEN: .txt and .pdf uploads via real OWUI → processing→completed, all 4 job stages (raw→markdown→chunked→owui_ingested) 1/1, chunks in stack Weaviate :18182 `Knowledge` collection with 1024-dim bge-m3 named vectors. Full reproduce recipe: `.superpowers/sdd/task-30-stack-report.md` + `docker-compose.stack-1-dp.yaml` (genai-utils worktree).
   **Key facts discovered**: (a) OWUI storage was switched to S3/MinIO (`STORAGE_PROVIDER=s3`, endpoint http://192.168.178.22:18195, path-style) because `LocalStorageProvider` has NO presigned GET — warren fetch REQUIRES S3-backed storage (tenant-relevant!); (b) env appended to OWUI worktree `.env` above the stack-manager marker; env is authoritative since `ENABLE_PERSISTENT_CONFIG=false`; host LAN IP used (host.docker.internal doesn't resolve from macOS host process); (c) pipeline-api requirements bumped warren 0.2.0→0.2.2 to match worker image (Mongo schema lockstep); (d) DESIGN FLAG: pipeline-api must set `final_data_type=owui_ingested` or jobs never complete (no embedder in the OWUI worker set); (e) DESIGN FLAG: publication worker has no ROLE in `entrypoint.py` — needed a compose-level entrypoint override.
...
1. ~~Verify/finish the stack-1-dp bring-up~~ DONE (see Task 2 above). Sanity: `docker compose -p stack-1-dp ps` — all services healthy; tmux `dp-logs` window tails them.
