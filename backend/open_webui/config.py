...
####################################
# Gradient-DS fork-custom config
#
# Env-var parses for ~100 fork-custom config entries, ported from the old
# per-key persistent-config machinery. Each persistent entry also has a dotted-path
# entry appended to DEFAULT_CONFIG below (see Config.configure at EOF).
# Env-var NAMES are byte-identical to the fork originals (helm requirement).
####################################

OAUTH_INVITE_REQUIRED = os.environ.get('OAUTH_INVITE_REQUIRED', 'False').lower() == 'true'

S3_VERIFY_SSL = os.getenv('S3_VERIFY_SSL', 'true').lower() == 'true'

ENABLE_ACCEPTANCE_MODAL = os.environ.get('ENABLE_ACCEPTANCE_MODAL', 'False').lower() == 'true'

ACCEPTANCE_MODAL_TITLE = os.environ.get('ACCEPTANCE_MODAL_TITLE', '')

ACCEPTANCE_MODAL_CONTENT = os.environ.get('ACCEPTANCE_MODAL_CONTENT', '')

ACCEPTANCE_MODAL_BUTTON_TEXT = os.environ.get('ACCEPTANCE_MODAL_BUTTON_TEXT', '')

GREETING_TEMPLATE = os.environ.get('GREETING_TEMPLATE', '')

ENABLE_WELCOME_MESSAGE = os.environ.get('ENABLE_WELCOME_MESSAGE', 'False').lower() == 'true'
...
ENABLE_FEEDBACK_LAYER2 = os.environ.get('ENABLE_FEEDBACK_LAYER2', 'True').lower() == 'true'

FEEDBACK_LAYER2_POSITIVE_TAGS = []

FEEDBACK_LAYER2_NEGATIVE_TAGS = []

ENABLE_FEEDBACK_LAYER3 = os.environ.get('ENABLE_FEEDBACK_LAYER3', 'True').lower() == 'true'

FEEDBACK_LAYER3_PROMPT = ''

ENABLE_FEEDBACK_CATEGORY_TAGS = os.environ.get('ENABLE_FEEDBACK_CATEGORY_TAGS', 'True').lower() == 'true'

ENABLE_CONVERSATION_FEEDBACK = os.environ.get('ENABLE_CONVERSATION_FEEDBACK', 'False').lower() == 'true'

CONVERSATION_FEEDBACK_SCALE_MAX = int(os.environ.get('CONVERSATION_FEEDBACK_SCALE_MAX', '5'))

CONVERSATION_FEEDBACK_HEADER = ''

CONVERSATION_FEEDBACK_PLACEHOLDER = ''

ENABLE_FEEDBACK_REPORTING = os.environ.get('ENABLE_FEEDBACK_REPORTING', 'False').lower() == 'true'
...
FEEDBACK_REPORT_TRACE_URL_TEMPLATE = os.environ.get('FEEDBACK_REPORT_TRACE_URL_TEMPLATE', '')
...
ENABLE_USER_ARCHIVAL = os.environ.get('ENABLE_USER_ARCHIVAL', 'True').lower() == 'true'

DEFAULT_ARCHIVE_RETENTION_DAYS = int(os.environ.get('DEFAULT_ARCHIVE_RETENTION_DAYS', '1095'))

ENABLE_AUTO_ARCHIVE_ON_SELF_DELETE = os.environ.get('ENABLE_AUTO_ARCHIVE_ON_SELF_DELETE', 'False').lower() == 'true'

AUTO_ARCHIVE_RETENTION_DAYS = int(os.environ.get('AUTO_ARCHIVE_RETENTION_DAYS', '365'))

ENABLE_2FA = os.environ.get('ENABLE_2FA', 'False').lower() == 'true'

REQUIRE_2FA = os.environ.get('REQUIRE_2FA', 'False').lower() == 'true'

TWO_FA_GRACE_PERIOD_DAYS = int(os.environ.get('TWO_FA_GRACE_PERIOD_DAYS', '7'))

ENABLE_DATA_WARNINGS = os.environ.get('ENABLE_DATA_WARNINGS', 'True').lower() == 'true'

DATA_RETENTION_TTL_DAYS = int(os.environ.get('DATA_RETENTION_TTL_DAYS', '0'))

USER_INACTIVITY_TTL_DAYS = int(os.environ.get('USER_INACTIVITY_TTL_DAYS', '0'))

CHAT_RETENTION_TTL_DAYS = int(os.environ.get('CHAT_RETENTION_TTL_DAYS', '0'))

KNOWLEDGE_RETENTION_TTL_DAYS = int(os.environ.get('KNOWLEDGE_RETENTION_TTL_DAYS', '0'))

DATA_RETENTION_WARNING_DAYS = int(os.environ.get('DATA_RETENTION_WARNING_DAYS', '30'))

ENABLE_RETENTION_WARNING_EMAIL = os.environ.get('ENABLE_RETENTION_WARNING_EMAIL', 'False').lower() == 'true'
...
ENABLE_CITATION_RELEVANCE = os.environ.get('ENABLE_CITATION_RELEVANCE', 'True').lower() == 'true'

ENABLE_CITATION_TEXT_HIGHLIGHT = os.environ.get('ENABLE_CITATION_TEXT_HIGHLIGHT', 'False').lower() == 'true'
...
EMAIL_FROM_ADDRESS = os.environ.get('EMAIL_FROM_ADDRESS', 'no-reply@soev.ai')

EMAIL_FROM_NAME = os.environ.get('EMAIL_FROM_NAME', 'Soev')

INVITE_EXPIRY_HOURS = int(os.environ.get('INVITE_EXPIRY_HOURS', '168'))

EMAIL_INVITE_SUBJECT = os.environ.get('EMAIL_INVITE_SUBJECT', '')

EMAIL_INVITE_HEADING = os.environ.get('EMAIL_INVITE_HEADING', '')

ENABLE_FORGOT_PASSWORD = os.environ.get('ENABLE_FORGOT_PASSWORD', 'False').lower() == 'true'
...
AGENT_SEARCH_ENABLED = os.environ.get('AGENT_SEARCH_ENABLED', 'False').lower() == 'true'

_default_selected_agent = AGENT_API_AGENTS[0] if AGENT_API_AGENTS else ''

AGENT_API_SELECTED_AGENT = _default_selected_agent

AGENT_API_PICKER_DEFAULT_SLUG = os.environ.get('AGENT_API_PICKER_DEFAULT_SLUG', '').strip()
...
DEFAULT_CONFIG = {
    ...
    # ── Gradient-DS fork-custom persistent config ──
    'admin.auto_archive_retention_days': AUTO_ARCHIVE_RETENTION_DAYS,
    'admin.chat_retention_ttl_days': CHAT_RETENTION_TTL_DAYS,
    'admin.data_retention_ttl_days': DATA_RETENTION_TTL_DAYS,
    'admin.data_retention_warning_days': DATA_RETENTION_WARNING_DAYS,
    'admin.default_archive_retention_days': DEFAULT_ARCHIVE_RETENTION_DAYS,
    'admin.enable_auto_archive_on_self_delete': ENABLE_AUTO_ARCHIVE_ON_SELF_DELETE,
    'admin.enable_retention_warning_email': ENABLE_RETENTION_WARNING_EMAIL,
    'admin.enable_user_archival': ENABLE_USER_ARCHIVAL,
    'admin.knowledge_retention_ttl_days': KNOWLEDGE_RETENTION_TTL_DAYS,
    'admin.user_inactivity_ttl_days': USER_INACTIVITY_TTL_DAYS,
    'agent_api.picker_default_slug': AGENT_API_PICKER_DEFAULT_SLUG,
    'agent_api.selected_agent': AGENT_API_SELECTED_AGENT,
    'agent_proxy.enable': ENABLE_AGENT_PROXY,
    'agent_search.enabled': AGENT_SEARCH_ENABLED,
    ...
    'email.invite_expiry_hours': INVITE_EXPIRY_HOURS,
    'email.invite_heading': EMAIL_INVITE_HEADING,
    'email.invite_subject': EMAIL_INVITE_SUBJECT,
    'email.password_reset_expiry_minutes': PASSWORD_RESET_EXPIRY_MINUTES,
    'evaluation.feedback.category_tags.enable': ENABLE_FEEDBACK_CATEGORY_TAGS,
    'evaluation.feedback.conversation.enable': ENABLE_CONVERSATION_FEEDBACK,
    'evaluation.feedback.conversation.header': CONVERSATION_FEEDBACK_HEADER,
    'evaluation.feedback.conversation.placeholder': CONVERSATION_FEEDBACK_PLACEHOLDER,
    'evaluation.feedback.conversation.scale_max': CONVERSATION_FEEDBACK_SCALE_MAX,
    'evaluation.feedback.layer2.enable': ENABLE_FEEDBACK_LAYER2,
    'evaluation.feedback.layer2.negative_tags': FEEDBACK_LAYER2_NEGATIVE_TAGS,
    'evaluation.feedback.layer2.positive_tags': FEEDBACK_LAYER2_POSITIVE_TAGS,
    'evaluation.feedback.layer3.enable': ENABLE_FEEDBACK_LAYER3,
    'evaluation.feedback.layer3.prompt': FEEDBACK_LAYER3_PROMPT,
    'features.enable_data_warnings': ENABLE_DATA_WARNINGS,
    ...
    'ui.enable_acceptance_modal': ENABLE_ACCEPTANCE_MODAL,
    'ui.enable_citation_relevance': ENABLE_CITATION_RELEVANCE,
    'ui.enable_citation_text_highlight': ENABLE_CITATION_TEXT_HIGHLIGHT,
    'ui.enable_welcome_message': ENABLE_WELCOME_MESSAGE,
    'ui.greeting_template': GREETING_TEMPLATE,
    'ui.soev_login_footer': SOEV_LOGIN_FOOTER,
}
