...
def introspect(client: httpx.Client, endpoint: str, out_path: str) -> Optional[dict]:
    ...
    kb_types = [
        t for t in types if t.get('name') and 'knowledge' in t['name'].lower() and not t['name'].startswith('__')
    ]
    if kb_types:
        ...
        _warn(
            "no type with 'knowledge' in its name — the KB query may be named differently; "
...
def main() -> int:
    ...
    ap.add_argument('--token', default=os.environ.get('TOPDESK_APP_PASSWORD', ''))
    ap.add_argument(
        '--login',
    ...
    ap.add_argument(
        '--graphql-path',
        default=os.environ.get('TOPDESK_GRAPHQL_PATH', ''),
    ...
    with httpx.Client(headers=headers, timeout=args.timeout, follow_redirects=False) as client:
        ...
        if endpoint:
            ...
            _warn(
                'Skipping introspection/queries — no GraphQL endpoint found. '
                "Find the path in the tenant's API explorer and pass --graphql-path."
            )

    _section('Done')
    print('  Review the output above against thoughts/shared/research/2026-06-topdesk-api-verification.md §10')
