...
def create_wrapper_content(package_path: str, agent_module: str, agent_metadata: Optional[Dict] = None) -> str:
    ...
    title = (
        agent_metadata.get('title', agent_module.replace('_', ' ').title())
        if agent_metadata
        else agent_module.replace('_', ' ').title()
    )
    description = (
        agent_metadata.get('description', f'External agent: {agent_module}')
        if agent_metadata
        else f'External agent: {agent_module}'
    )
...
async def register_external_agent_direct(agent_id: str, wrapper_content: str, user_id: str = 'system') -> bool:
    ...
    try:
        ...
        function_module, function_type, _ = await load_function_module_by_id(agent_id, content=wrapper_content)
        ...
        form_data = FunctionForm(
            ...
            meta=FunctionMeta(description=function_description, manifest=frontmatter),
        )
