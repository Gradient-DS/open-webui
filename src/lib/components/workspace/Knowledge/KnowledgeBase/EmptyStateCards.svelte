		...
		{#each options as option}
			<button
				class="flex flex-col items-center justify-center gap-3 p-8
					...
					rounded-2xl {option.type === 'integration'
					? 'cursor-default'
					: 'cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-850 hover:border-gray-400 dark:hover:border-gray-500'}
					transition-all duration-150"
				on:click={() => {
					if (option.type !== 'integration') onAction(option.type);
				}}
			>
