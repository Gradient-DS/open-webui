...
def call_external_pipeline(
    ...
    The external pipeline then performs:
    - Parsing (page marker detection)
    - Chunking (recursive character splitting)
    - Formatting (preparing chunks for embedding)
    
    Expected external pipeline API:
    - Endpoint: POST /process-document
    - Input: Multipart form data with:
        * file: UTF-8 encoded text bytes with optional page markers
    ...
    Note: Embeddings are generated by Open WebUI after receiving chunks.
    
    Args:
        file_path: Path to the file
        filename: Original filename
        content_type: MIME type of the file (used to determine file type)
                ...
                if file_path:
                    ...
                    if use_external_pipeline:
                        ...
                        try:
                            ...
                            # Create loader for document loading (text extraction)
                            # This will be used by call_external_pipeline to load the document
                            loader = Loader(
