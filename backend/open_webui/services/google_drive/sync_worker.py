...
class GoogleDriveSyncWorker(BaseSyncWorker):
    ...
    def max_files_config(self) -> Optional[int]:
        # GOOGLE_DRIVE_MAX_FILES_PER_SYNC is a PersistentConfig (admin-editable).
        # 0 = "no limit" → return None so base_worker falls back to the
        # KB-wide KNOWLEDGE_MAX_FILE_COUNT safety net alone.
        return GOOGLE_DRIVE_MAX_FILES_PER_SYNC.value or None
        ...
        try:
            ...
            if current_indicator and current_indicator == stored_indicator:
                ...
                if existing and (existing.data or {}).get('status') == 'completed':
                    ...
                    log.info(f'File {source["name"]} indicator matches but record {reason}, re-syncing')
    ...
    async def _verify_source_access(self, source: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if e.response.status_code in (403, 404):
                log.warning(f'User {self.user_id} lost access to {source_type} {item_id}: {e.response.status_code}')
                return False
