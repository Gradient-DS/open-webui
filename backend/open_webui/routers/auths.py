...
ADMIN_CONFIG_KEYS = {
    ...
    # Gradient-DS fork toggles — must be mapped so the admin key-map round-trips
    # them (get_config_values / config_updates only touch mapped keys).
    'ENABLE_CITATION_RELEVANCE': 'ui.enable_citation_relevance',
    'ENABLE_ACCEPTANCE_MODAL': 'ui.enable_acceptance_modal',
    'ACCEPTANCE_MODAL_TITLE': 'ui.acceptance_modal_title',
    'ACCEPTANCE_MODAL_CONTENT': 'ui.acceptance_modal_content',
    'ACCEPTANCE_MODAL_BUTTON_TEXT': 'ui.acceptance_modal_button_text',
    'ENABLE_WELCOME_MESSAGE': 'ui.enable_welcome_message',
}
...
async def evaluate_2fa_grace(request: Request, user, auth_record, db) -> dict:
    ...
    """
    if not (await Config.get('auth.enable_2fa') and await Config.get('auth.require_2fa')):
    ...
    grace = compute_twofa_grace(started, await Config.get('auth.2fa_grace_period_days'), now)
...
async def forgot_password(
    ...
    try:
        eligible = (
            await Config.get('email.enable_forgot_password')
        ...
        if eligible:
            ...
            if user:
                ...
                if auth and auth.active:
                    ...
                    expiry_minutes = int(await Config.get('email.password_reset_expiry_minutes'))
...
async def reset_password(
    ...
    if not await Config.get('email.enable_forgot_password'):
...
async def signin(
    ...
    if user:
        ...
        if await Config.get('auth.enable_2fa'):
...
async def signup(
    ...
    try:
        ...
        return await create_session_response(request, user, db, response, set_cookie=True, expires_at_cap=grace_cap)
    except HTTPException:
        raise
    except Exception as err:
...
class AdminConfig(BaseModel):
    ...
    ENABLE_CITATION_RELEVANCE: bool
    ...
    ENABLE_ACCEPTANCE_MODAL: bool = False
    ACCEPTANCE_MODAL_TITLE: str | None = None
    ACCEPTANCE_MODAL_CONTENT: str | None = None
    ACCEPTANCE_MODAL_BUTTON_TEXT: str | None = None
