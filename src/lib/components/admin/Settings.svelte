<script>
	...
	const setFilteredSettings = () => {
		filteredSettings = allSettings.filter((tab) => {
			// First check if the tab is enabled via feature flags
			if (!isAdminSettingsTabEnabled(tab.id)) {
				return false;
			}
	...
	// Re-filter when config changes (feature flags may update)
	$: if ($config) {
		setFilteredSettings();
	}
